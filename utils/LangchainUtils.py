...
from langchain_core.output_parsers import (
    # For parsing JSON responses from LLMs
    JsonOutputParser,
)
...
from langchain_core.prompts import (
    ...
    PromptTemplate as PromptTemplateLangchain,
)
...
class parser_recommendation(BaseModel):
    ...
    movie_list: str = Field(
        description="Re-ranked movie list, only movie title, separate by '|'"
    )
...
def call_llm_summarize(document: str, gen_model: str, api_key: str, recommended_item: str) -> Dict[str, str]:
    """
    ...
    time.sleep(10)
    output = chain_llm_summarize(gen_model, api_key).invoke({"document": document, 'item': recommended_item})
...
def chain_llm_reranking(gen_model: str, api_key: str) -> LLMChain:
    ...
    prompt = PromptTemplateLangchain(
        ...
        input_variables=[
            # "document",
            "summary_preference", "movie_list", "k"
        ],
        # Add formatting instructions for JSON output
        partial_variables={"format_instructions": parser.get_format_instructions()},
    )
