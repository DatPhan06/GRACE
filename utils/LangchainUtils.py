...
import time
from langchain_core.output_parsers import JsonOutputParser                      # For parsing JSON responses from LLMs
from langchain_google_genai import ChatGoogleGenerativeAI                       # Google's generative AI integration
from langchain_core.prompts import PromptTemplate as PromptTemplateLangchain    # For creating structured prompts
...
class parser_recommendation(BaseModel):
    ...
    movie_list: str = Field(description="Re ranking movie list, only movie title, separate by '|'")  
...
def chain_llm_summarize(gen_model: str, api_key: str) -> LLMChain:
    ...
    # Few-shot
    examples = """
    - Exapmle 1: The seeker is looking for a good action comedy and is tired of holiday movies. 
    He/she doesn't like superhero movies but do enjoy British comedies like Red Dwarf. 
    He/she is interested in watching Hot Fuzz, especially since it stars Simon Pegg from Shaun of the Dead. 
    He/she also enjoyed Zombieland and Zombieland 2, with Woody Harrelson being a favorite.
    - Example 2: The seeker enjoys comedy and horror movies, particularly R-rated ones. 
    His/her favorite actors include Seth Rogan and Seth MacFarlane. 
    He/she recently watched and enjoyed the movie 'Ted'. 
    He/she is potentially interested in the movie 'Superbad' and inquired about its rating and if it contains nudity, 
    indicating a preference for content without explicit nudity.
    - Example 3: The seekr is interested in fantasy or animated movies. 
    He/she have watched Frozen 2. 
    He/she are concerned about violence and age appropriateness for his/her niece. 
    He/she accepted a recommendation for an animated movie about a dragon and a boy, 
    with a PG age rating, produced by 20th Century Fox, released in 2014, 
    and 104 minutes long (How to Train Your Dragon 2). They prefer family-friendly movies.
    """
    ...
    prompt = PromptTemplateLangchain(
        ...
        Read this conversation, find all the information about the seeker's preferences in movie, actor, genres, countries and content (Do not contain assistant preferences), and summarize them.
...
def call_llm_summarize(document: str, gen_model: str, api_key: str) -> Dict[str, str]:
    """
    ...
    time.sleep(10)
    output = chain_llm_summarize(gen_model, api_key).invoke({"document": document})  
    
    return output


#------------------- RE-RANKING OUTPUT ------------------------------
def chain_llm_reranking(gen_model: str, api_key: str) -> LLMChain:
    """
    ...
    parser = JsonOutputParser(pydantic_object=parser_recommendation)  
    ...
    prompt = PromptTemplateLangchain(
        ...
        Read the conversation, summary of seeker's preferences, and candidate list, 
        then re-rank movie candidate list and retrieve top 50 movies that match the seeker's preferences the most.
        If the candidate list is empty, you dont't need to do anything.
        Let's think's step by step.
        {format_instructions}
        """,  
        
        # Define variables that will be replaced in the template
        input_variables=["document", "summary_preference", "movie_list"],  
...
def call_llm_reranking(context_str: str, user_preferences: str, movie_str: str, 
                      gen_model: str, api_key: str) -> Dict[str, str]:
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            chain = chain_llm_reranking(gen_model, api_key)
            output = chain.invoke(
                {"document": context_str, "summary_preference": user_preferences, "movie_list": movie_str})
            return output
    ...
    # Create and invoke chain with all required inputs
    # output = chain_llm_reranking(gen_model, api_key).invoke(
    #     {"document": context_str, "summary_preference": user_preferences, "movie_list": movie_str})  
