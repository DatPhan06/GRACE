...
# ------------------- OPTIMIZED RE-RANKING FUNCTIONS ------------------------------

def callOptimizedReranking_redial(
    context: Any,
    ...
    """
    Optimized reranking function that addresses the issues with naive LLM reranking.
    
    This function uses a combination of strategies:
    1. Semantic filtering to remove obviously mismatched movies
    2. Hierarchical reranking to handle large candidate sets
    3. Hybrid scoring to combine cosine similarity with LLM preferences
    4. Smart batching to optimize token usage
    
    Args:
        context: The conversation text for context
        user_preferences: Summarized user preferences
        movie_list: List of candidate movies to re-rank
        movie_data_path: Path to movie metadata file
        model: The generative model name/identifier
        api_key: List of API keys for authentication
        cosine_scores: Optional cosine similarity scores for hybrid scoring
        k: Top k movies to return
    
    Returns:
        Parsed re-ranked list of movies with confidence score
    """
    
    try:
        from .optimized_reranking import adaptive_reranking_strategy
        
        result = adaptive_reranking_strategy(
        ...
        return {
            "movie_list": result.get("movie_list", []),
        ...
        logging.error(f"Optimized reranking failed: {e}")
        # Fallback to original reranking
        try:
            fallback_result = callLangChainLLMReranking_redial(
                context=context,
                user_preferences=user_preferences,
            ...
            return {
                "movie_list": fallback_result.get("movie_list", movie_list[:k]),
                "confidence": 0.3,
                "method_used": "fallback_original_reranking",
                "statistics": {"fallback": True}
            }
        except Exception as fallback_error:
            logging.error(f"Fallback reranking also failed: {fallback_error}")
            return {
                "movie_list": movie_list[:k],
                "confidence": 0.1,
                "method_used": "fallback_truncation",
                "statistics": {"fallback": True, "error": str(fallback_error)}
            }

# ------------------- ORIGINAL RE-RANKING (LEGACY) ------------------------------
...
<instruction>
Your task is to re-rank a given list of candidate movies based on a summary of a user's preferences and the full context of their conversation. You need to return the top {k} movies that are most relevant to the user's tastes.
</instruction>
...
Candidate Movie List: {movie_list}
</input>
