"""
Run ARGOS evaluation on INSPIRED test dataset.

Generates TSV output files in output/agros/INSPIRED/ with the same format as
output/REDIAL/ and output/INSPIRED/ (legacy GRACE).
...
Pre-requisites (run once before this script):
  1. Neo4j running and populated with INSPIRED KG:
       cd backend && python scripts/graph_builder_inspired.py
  2. Vector embeddings built:
       cd backend && python scripts/generate_embedding.py --dataset inspired
  3. .env configured with GEMINI_API_KEY, OPENAI_API_KEY, AWS credentials

Usage (from project root):
  cd backend
  python scripts/run_argos_inspired_eval.py
  python scripts/run_argos_inspired_eval.py --sample 50
  python scripts/run_argos_inspired_eval.py --sample 100 --concurrency 3 --model decoupled
  python scripts/run_argos_inspired_eval.py --top-ks 1 10 50

Options:
  --sample N        Number of test dialogs to evaluate (default: all 267 from test set)
...
INSPIRED_TEST_DIALOGS = PROJECT_ROOT / "dataset/INSPIRED/processed/dialog_data/test_new_processed.json"
OUTPUT_DIR = PROJECT_ROOT / "output/agros/INSPIRED"
MODEL_LABEL = "gemini-2.0-flash"

# Words to exclude from liked-movies heuristic extraction
_STOP_WORDS = {
...
def dialog_to_context(dialog_turns: List[str]) -> str:
    """Join dialog turn list into a single context string."""
    return "\n".join(dialog_turns)


def extract_liked_movies(dialog_turns: List[str]) -> List[str]:
    """Heuristic: extract capitalised phrases that look like movie titles."""
...
def load_done_ids(output_paths: Dict[int, Path]) -> set:
    """Return set of conv ids already written to ALL output files (for --resume)."""
    done_sets = []
    for path in output_paths.values():
        if path.exists():
            ...
            return set()  # At least one file missing — start fresh
    if not done_sets:
        return set()
...
# ---------------------------------------------------------------------------
# Core ARGOS pipeline (mirrors EvaluationService.process_single_conversation)
# ---------------------------------------------------------------------------
...
async def process_dialog(
    ...
    try:
        ...
        # 3. Critic → Relaxation loop (max 1 relaxation attempt)
        MAX_RELAX = 1
        ...
        for attempt in range(MAX_RELAX + 1):
            ...
            # Relaxation: widen constraints and re-retrieve (no re-profiling)
        ...
        # 4. Decoupled Reranker — run once with max K, truncate later per K
...
async def main(
    ...
    dialogs = load_test_dialogs(INSPIRED_TEST_DIALOGS, sample_size, seed)
    actual_n = len(dialogs)
    print(f"Loaded {actual_n} INSPIRED test dialogs from {INSPIRED_TEST_DIALOGS.name}")
    ...
    # Filter out already-done dialogs
    todo = [
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run ARGOS multi-agent evaluation on INSPIRED test dataset",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    parser.add_argument("--sample", type=int, default=None,
                        help="Dialogs to evaluate (default: all 267 from test set)")
