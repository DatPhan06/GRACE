...
async def main():
    ...
    if builder.driver:
        ...
        print(f"[INFO] Bắt đầu xử lý {len(all_movies)} phim INSPIRED với giới hạn đồng thời là {CONCURRENCY_LIMIT}...")
        
        for i, task in enumerate(asyncio.as_completed(tasks)):
            await task
            print(f"({i + 1}/{len(all_movies)}) Đã xử lý phim INSPIRED xong.")

        print(f"\nĐã chèn xong {len(all_movies)} phim INSPIRED vào Neo4j.")
        builder.close()
    else:
        print("Không thể kết nối đến Neo4j. Vui lòng kiểm tra cấu hình.")
