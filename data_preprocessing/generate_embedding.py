...
from llama_index.core import Document                           # Document container for text data
from llama_index.core import StorageContext                     # Context for managing storage backends
from llama_index.core import VectorStoreIndex                   # Index for vector retrieval operations
...
def get_document_list(movie_data_path: os.PathLike) -> List[Document]:
    ...
    """
    # Load all movie data from the JSONL file
    movie_data = []
    for line in open(movie_data_path, "r"):
        movie_data.append(json.loads(line))
        
    # Create documents for each valid movie entry
    documents = [] 
    for i in range(len(movie_data)):
        ...
        if len(df_current_movie) > 2:  
            ...
            documents.append(Document(text=embedding_string,
                                      doc_id=str(df_current_movie['movieId'])))  
    return documents


def create_embedding_db(db_path: str, 
                        collection_name: str, 
                        embedding_model: str, 
                        api_key: str, 
                        movie_data_path: os.PathLike) -> None:
    """
...
# create_embedding_db(db_path='dataset/embedding/redial_movie_embedding',
#                     collection_name='redialMovie',
#                     embedding_model='models/text-embedding-004',
#                     api_key='AIzaSyAgGyG5MbSXWyr43kooFL_xqcrqGPmGVPo',
#                     movie_data_path='dataset/preprocessed_data/ReDial/movie_data/movie.jsonl')