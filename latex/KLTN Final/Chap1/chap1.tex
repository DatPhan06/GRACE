...
Sự trưởng thành của các Mô hình ngôn ngữ lớn (Large Language Models - LLMs) mở ra cơ hội lớn cho CRS nhờ năng lực hiểu và sinh ngôn ngữ tự nhiên vượt trội. Tuy nhiên, ứng dụng trực tiếp LLM vào CRS bộc lộ hai rào cản cốt lõi. Về tính tin cậy, LLM sinh văn bản theo phân phối xác suất chứ không truy xuất dữ liệu thực tế, khiến chúng dễ phát sinh hiện tượng "ảo giác" (hallucination), tự bịa ra các thực thể không tồn tại như tên phim, đạo diễn hay năm phát hành. Về hiệu quả vận hành, việc tinh chỉnh LLM cho mỗi thay đổi dữ liệu tốn kém tài nguyên; ngược lại, nếu đưa toàn bộ tri thức vào context window thì chi phí API và độ trễ tăng vọt, ảnh hưởng trực tiếp đến khả năng phản hồi thời gian thực.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{@{}
        ...
        \cmidrule{2-5}
                           & \textbf{GRACE (Baseline)}                                                                              & \textbf{Truy xuất lai (Sem+Content+Graph) kết hợp LLM Re-ranker.} & \textbf{ReDial, INSPIRED} & \textbf{Không}          \\
        \bottomrule
    \end{tabular}
\end{table}
...
\textbf{(2) Thiếu khả năng thỏa hiệp ràng buộc (Constraint Relaxation):} Khi người dùng đặt ra các điều kiện mâu thuẫn hoặc quá hạn hẹp, chúng không thể tự quyết định nên nới lỏng ràng buộc nào để đưa ra gợi ý xấp xỉ; thay vào đó chúng trả về kết quả rỗng, một trải nghiệm người dùng rất kém.

Trong khi đó, nghiên cứu về kiến trúc đa tác tử dựa trên LLM \cite{xi2023rise} đã chứng minh rằng việc phân tách hệ thống thành các tác tử chuyên biệt với vòng lặp phản hồi (feedback loop) có thể xử lý hiệu quả các bài toán đòi hỏi suy luận đa bước và tự sửa lỗi. Tuy nhiên, hướng tiếp cận này chưa được áp dụng vào bài toán CRS kết hợp Đồ thị tri thức, đây chính là khoảng trống mà khóa luận đề xuất lấp đầy thông qua hệ thống \textbf{ARGOS}.

\section{Mục tiêu nghiên cứu}

Khóa luận đặt ra bốn mục tiêu nghiên cứu:
\begin{itemize}
    \item \textbf{Mục tiêu 1:} Phân tích và định lượng các điểm nghẽn của kiến trúc GRACE trong bài toán CRS để tạo cơ sở thực nghiệm cho việc đề xuất cơ chế mới.
    \item \textbf{Mục tiêu 2:} Thiết kế kiến trúc đa tác tử ARGOS với cơ chế phân phối nhiệm vụ và vòng lặp phản hồi nội bộ, giải quyết các hạn chế đó mà không yêu cầu huấn luyện lại mô hình.
...
Khóa luận mang lại bốn đóng góp kỹ thuật mới cho lĩnh vực CRS:
\begin{enumerate}
    \item \textbf{Kiến trúc đa tác tử thích nghi ARGOS:} Lần đầu tiên áp dụng mô hình Multi-Agent với vòng lặp suy luận-hành động vào hệ thống CRS kết hợp Đồ thị tri thức. ARGOS phân tách pipeline thành năm tác tử chuyên biệt (Profiler Agent, Orchestrator, Graph Agent, Critic Agent, Relaxation Agent) và sử dụng Dynamic Weighting để điều phối chiến lược truy xuất theo hình thái ngôn ngữ của truy vấn — cơ chế chưa xuất hiện trong các hệ thống CRS prior work.
    \item \textbf{Cơ chế kiểm định xuyên luồng (Critic Agent):} Phương pháp đối soát chéo độc lập, đảm bảo mọi ứng viên từ tất cả các luồng truy xuất đều được kiểm định theo ràng buộc tường minh trước khi xếp hạng. Critic Agent lập luận trên metadata thực tế từ Đồ thị tri thức, không dựa vào bộ nhớ tham số của LLM, triệt tiêu hallucination ở tầng kết quả.
    \item \textbf{Cơ chế tự sửa lỗi và thỏa hiệp thông minh (Relaxation Agent):} Khi tập ứng viên hợp lệ không đủ ngưỡng, Relaxation Agent áp dụng thuật toán Phân cấp ưu tiên (Sacrifice Hierarchy) — dựa trên nhãn ưu tiên được Profiler gán sẵn — để nới lỏng tối thiểu ràng buộc gây tắc nghẽn và tái cấu trúc ý định mà không cần gọi thêm LLM. Đây là đóng góp kỹ thuật độc lập, có thể tách rời và ứng dụng cho các hệ thống RAG khác ngoài CRS.
    \item \textbf{Tối ưu hóa độ trễ qua Xếp hạng hai giai đoạn tách rời (Decoupled Reranking):} Thay thế LLM ở giai đoạn lọc đầu bằng Cross-Encoder chuyên dụng (SLM), chỉ dùng Generative LLM ở giai đoạn suy luận cuối trên Top-20 ứng viên. Cơ chế này duy trì hiệu năng gợi ý cạnh tranh trong khi giảm đáng kể chi phí tính toán và độ trễ end-to-end.
\end{enumerate}

ARGOS cải thiện đáng kể so với kiến trúc tiền đề GRACE và đạt kết quả cạnh tranh với các hệ thống tiên tiến trên ReDial và INSPIRED về chỉ số Recall@K. Toàn bộ mã nguồn và dữ liệu thực nghiệm được công bố công khai tại \url{https://github.com/DatPhan06/GRACE}, tạo nền tảng cho các nghiên cứu tiếp theo trong lĩnh vực.
