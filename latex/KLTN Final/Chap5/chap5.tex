...
Khi đối chiếu với các hệ thống được tối ưu hóa qua quá trình huấn luyện như DCRS, GRACE duy trì mức hiệu suất ổn định dù sử dụng phương pháp không cần huấn luyện (training-free). DCRS ghi nhận mức cao hơn tại Recall@1, chủ yếu do sự tương đồng mật thiết (overfit) với phân phối dữ liệu của tập huấn luyện. Tuy nhiên, GRACE thể hiện khả năng tổng quát hóa tốt hơn khi đánh giá ở các ngưỡng sâu trên ReDial (Recall@50 đạt 0.527 so với 0.439 của DCRS) và duy trì mức hiệu năng ổn định trên tập INSPIRED.
...
\subsection{Tác tử Kiểm định (Critic Agent): Kiểm định ràng buộc xuyên luồng}
\textbf{Truy vấn người dùng:} \textit{"Phim hoạt hình nhẹ nhàng cho trẻ em, có robot giống Wall-E nhé."}
\begin{itemize}
    \item \textbf{Hệ thống tĩnh:} Từ khóa "robot" có thể khiến cơ chế truy xuất ngữ nghĩa vô tình đưa \textit{The Terminator} vào danh sách ứng viên. Luồng Semantic Retrieval không có kiến thức về ràng buộc đối tượng khán giả, và do thiếu tầng kiểm định độc lập, mô hình xếp hạng có thể ưu tiên ứng viên này vì điểm tương đồng vector cao.
    \item \textbf{Hệ thống ARGOS (Tác tử Kiểm định):} Tác tử đối chiếu từng ứng viên với tập Ràng buộc cứng và phát hiện vi phạm: \textit{"Yêu cầu chỉ định đối tượng 'trẻ em', trong khi The Terminator mang nhãn phân loại R. Vi phạm ràng buộc về phân loại độ tuổi"}. Tác tử loại bỏ ứng viên vi phạm trước khi chuyển sang giai đoạn xếp hạng, chỉ giữ lại các lựa chọn phù hợp như \textit{Big Hero 6} hoặc \textit{Next Gen}. Đây là ví dụ điển hình của kiểm định ràng buộc xuyên luồng: luồng Semantic Retrieval trả về ứng viên theo vector nhưng không lọc theo ràng buộc; Critic Agent là tầng duy nhất đảm bảo toàn bộ ứng viên đưa ra xếp hạng đều thỏa mãn các điều kiện cứng của người dùng.
\end{itemize}
