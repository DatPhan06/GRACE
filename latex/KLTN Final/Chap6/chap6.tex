...
\begin{enumerate}
  \item \textbf{Cải thiện hiệu năng có kiểm chứng thống kê:} Trên tập mẫu đánh giá (20\%, $p < 0.01$), ARGOS vượt qua mô hình tiền đề GRACE trên toàn bộ các ngưỡng đo lường của cả hai bộ dữ liệu ReDial và INSPIRED, và đạt kết quả cạnh tranh so với các mô hình tinh chỉnh hàng đầu như DCRS, dù lưu ý rằng so sánh với SOTA chỉ mang tính định hướng do sự khác biệt về kích thước tập kiểm thử.
  ...
  \item \textbf{Giảm thiểu ảo giác và tăng độ tin cậy:} Critic Agent hoạt động như một cổng kiểm định độc lập, đối chiếu siêu dữ liệu của từng ứng viên với tập Ràng buộc cứng do người dùng đặt ra, loại bỏ các ứng viên vi phạm trước khi chuyển sang giai đoạn xếp hạng.
  \item \textbf{Cải thiện sự thông suốt của mạch hội thoại:} Relaxation Agent giải quyết tình huống kết quả rỗng đối với các yêu cầu quá khắt khe, thông qua cơ chế tự động nới lỏng ràng buộc theo phân cấp ưu tiên và diễn giải logic, giúp duy trì mạch hội thoại liên tục mà không cần can thiệp từ người dùng.
...
\section{Mối quan hệ với công trình tiền đề GRACE}

Khóa luận này kế thừa và mở rộng đáng kể so với bài báo GRACE \cite{phan2025grace} đã được công bố. Bảng dưới đây phân định rõ ranh giới giữa những gì có trong bài báo và những đóng góp kỹ thuật mới được phát triển trong phạm vi khóa luận:

\begin{table}[H]
  \centering
  \caption{Phân định đóng góp giữa bài báo GRACE (SOICT 2025) và khóa luận ARGOS.}
  \begin{tabular}{p{0.45\linewidth} p{0.45\linewidth}}
    \toprule
    \textbf{Bài báo GRACE (tiền đề)} & \textbf{Khóa luận ARGOS (mới)} \\
    \midrule
    Kiến trúc Hybrid Retrieval (Semantic + Content + Graph) & Profiler Agent với phân rã ý định đa nhánh và Multi-Query Expansion \\
    \addlinespace
    Công thức WRRF với trọng số tĩnh & Cơ chế Dynamic Weighting: trọng số thích nghi theo ngữ cảnh hội thoại \\
    \addlinespace
    Validator JSON schema đầu ra & Critic Agent: kiểm định ràng buộc cứng xuyên luồng và tích hợp vòng lặp phản hồi \\
    \addlinespace
    Two-stage batching re-ranking (LLM cho cả hai giai đoạn) & Decoupled Reranking: Cross-Encoder chuyên dụng (Stage 1) kết hợp LLM suy luận ngữ nghĩa (Stage 2) \\
    \addlinespace
    (Không có cơ chế phục hồi kết quả rỗng) & Relaxation Agent: tự động nới lỏng ràng buộc theo phân cấp và tái cấu trúc ý định \\
    \addlinespace
    Pipeline tuyến tính một chiều & Kiến trúc đa tác tử với vòng lặp phản hồi ReAct (Multi-Agent Feedback Loop) \\
    \bottomrule
  \end{tabular}
\end{table}

Nói cách khác, bài báo GRACE đặt nền tảng cho cơ chế truy xuất lai và xếp hạng; khóa luận này xây dựng toàn bộ lớp điều phối đa tác tử phía trên, bao gồm khả năng tự phân tích ý định, tự kiểm định kết quả và tự phục hồi khi thất bại.
