...
RAG đặc biệt phù hợp với bài toán CRS vì ba lý do thực tiễn: \textit{(i)} danh mục item thay đổi liên tục: RAG chỉ cần cập nhật KG mà không cần fine-tune lại LLM; \textit{(ii)} mỗi gợi ý đều cần có cơ sở thực chứng: đường dẫn truy xuất của RAG cung cấp trực tiếp khả năng giải thích (explainability); \textit{(iii)} chi phí thấp hơn đáng kể so với fine-tuning khi quy mô dữ liệu lớn.
...
Nghiên cứu này sử dụng hai thang đo tiêu chuẩn trong lĩnh vực gợi ý để đánh giá module Recommendation của ARGOS: \textbf{Recall@K} và \textbf{NDCG@K}.
...
\textbf{NDCG@K} (Normalized Discounted Cumulative Gain) bổ sung chiều thứ hạng: item đúng xuất hiện càng sớm trong danh sách được thưởng điểm cao hơn:
\begin{equation}
    \text{DCG@K} = \sum_{i=1}^{K} \frac{rel_i}{\log_2(i+1)}, \quad \text{NDCG@K} = \frac{\text{DCG@K}}{\text{IDCG@K}}
\end{equation}
trong đó $rel_i = 1$ nếu item ở vị trí $i$ thuộc ground truth, ngược lại $= 0$; IDCG là giá trị DCG lý tưởng khi tất cả item đúng đều được xếp hàng đầu. Các thang đo này được tính trên hai bộ dữ liệu chuẩn ReDial \cite{li2018towards} và INSPIRED \cite{hayati-etal-2020-inspired}.

\section*{Kết luận chương}

Chương 2 đã thiết lập bốn nhóm kiến thức cốt lõi phục vụ cho việc hiểu và đánh giá hệ thống ARGOS. \textit{Bài toán CRS} xác lập không gian vấn đề với hai nhiệm vụ song song mà ARGOS phải giải quyết. \textit{Đồ thị tri thức} là nguồn dữ liệu thực chứng trung tâm, đảm bảo mọi gợi ý đều có cơ sở kiểm chứng và là nền tảng cho Hybrid Retrieval. \textit{LLM kết hợp RAG} cung cấp năng lực suy luận ngôn ngữ mà không cần fine-tuning khi dữ liệu thay đổi. \textit{Kiến trúc đa tác tử với ReAct} là mô hình tổ chức cho phép thiết lập vòng lặp phản hồi nội bộ, thành phần then chốt mà các hệ thống RAG tĩnh đang thiếu, và là nền tảng kỹ thuật của ARGOS.
